...
// #endregion
  ...
  SetImageColumn(ImageColumnId: number) {

    var found = 0;
    var self = this;

    self.ImageCols.forEach(element => {
  ...
  ClearImageColumn(ImageColumnId: number) {
    var found = 0;

    var self = this;

    var tempimagecols: number[] = [];
  ...
  SetScrollMultiplier(Multiplier: number) {
      ...
      for (var _curcol = 0;_curcol<this.GridRows[_currow].length;_curcol++)
      {
        ...
        if (this.AlternateRowColoring)
        {
          ...
          else
          {
            ...
            {
              if (_currow == this.SelectedRow || this.SelectedRows.indexOf(_currow) >= 0)
              {
        ...
        this.ImageCols.forEach(element => {
          if (element == _curcol)
          {
            ...
            ctx.drawImage(this.Images[0],lx + 3 - this.HorizontalOffset, ly - this.VerticleOffset)
            imgdrawn = true;
  ...
  HandleMouseWheel = (ev: WheelEvent) => {
    ...
    if (this.VerticleScrollBarVisible && !ev.ctrlKey) {
      ...
      this.VerticleOffset += (ev.deltaY/2) * this.ScrollMultiplier;
    ...
    else
    {
      if (this.HorizontalScrollBarVisible) {
        ...
        this.HorizontalOffset += ev.deltaY * this.ScrollMultiplier ;
    ...
    if (!this.ScrollButtonDown)
    {
      ...
      if (therow !=-1 && thecol != -1)
      {
        ...
        if (therow === this.SelectedRow)
        {
          ...
          {
            this.SelectedRow = -1;
            this.RemoveRowFromSelectedRows(therow);
          }
          else
          {
            this.SelectedRow = -1;
        ...
        else
        {
          ...
          if (this.SelectedRows.indexOf(therow) >= 0)
          {
            if (ev.shiftKey)
            {
              this.RemoveRowFromSelectedRows(therow);
  ...
  private RemoveRowFromSelectedRows(element: number) {
